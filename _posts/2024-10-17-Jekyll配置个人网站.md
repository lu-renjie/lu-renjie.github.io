...
参考[这里](https://be-my-only.xyz/blog/TeXt-copy-to-clipboard/)。照着做没有用，最后发现需要把copy-to-clipboard.js里的copyable改成highlight，并且把添加的sass文件里的`.copyable`也改成`.highlight`才行。
